...
A connector for the thorlabs motor controller built with the UniteLabs CDK.
...
```sh
set UNITELABS_CDK_APP=unitelabs.thorlabs_motor_controller:create_app
```

Setting this environment variable will allow you to run varous CLI commands without providing `--app unitelabs.thorlabs_motor_controller:create_app` every time.
...
```sh
python -m pip install unitelabs-thorlabs-motor-controller \
...
- On **Windows**:
  ...
  ```sh
  config show --app unitelabs.thorlabs_motor_controller:create_app
  ```
...
- On **Windows**:
  ...
  ```sh
  config create --app unitelabs.thorlabs_motor_controller:create_app
  ```
...
- On **Windows**:
  ...
  ```sh
  config create --app unitelabs.thorlabs_motor_controller:create_app  --path <path to config>
  ```
...
```sh
connector start --app unitelabs.thorlabs_motor_controller:create_app -vvv
```
...
```sh
connector start --app unitelabs.thorlabs_motor_controller:create_app -cfg <path to config> -vvv
```
...
```sh
git clone diao.scs.illinois.edu.git
```
...
```sh
uv run connector dev --app unitelabs.thorlabs_motor_controller:create_app
```
...
[issue-tracker]: diao.scs.illinois.edu/issues
